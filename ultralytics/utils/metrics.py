...
class Metric(SimpleClass):
    ...
    def __init__(self) -> None:
        ...
        self.nc = 0
        self.nt = None # OWN MODIFICATION: number of instances per class
    ...
    def _weighted_mean(self, arr): # OWN MODIFICATION: added function to calculate weighted mean
        """Calculate the weighted mean of an array using the number of instances per class."""
        if self.nt is not None and len(arr) == len(self.nt):
            total = self.nt.sum()
            if total > 0:
                return (arr * self.nt).sum() / total
        return arr.mean() if len(arr) else 0.0
    
    def _mean(self, arr, weighted=False):
        """
        Return the mean of an array, weighted or unweighted.
        """
        if weighted:
            return self._weighted_mean(arr)
        else:
            return arr.mean() if len(arr) else 0.0
    ...
    def mp(self):
        ...
        """
        # OWN MODIFICATION: changed to use weighted mean
        return self._mean(self.p, weighted=False) if len(self.p) else 0.0
        # ONLY WEIGHTED VERSION:
        # return self._weighted_mean(self.p) if len(self.p) else 0.0
        # OLD VERSION:
    ...
    def mr(self):
        ...
        """
        # OWN MODIFICATION: changed to use weighted mean
        return self._mean(self.r, weighted=False) if len(self.r) else 0.0
        # ONLY WEIGHTED VERSION:
        # return self._weighted_mean(self.r) if len(self.r) else 0.0
        # OLD VERSION:
    ...
    def map50(self):
        ...
        """
        # OWN MODIFICATION: changed to use weighted mean
        return self._mean(self.all_ap[:, 0], weighted=False) if len(self.all_ap) else 0.0
        # ONLY WEIGHTED VERSION:
        # return self._weighted_mean(self.all_ap[:, 0]) if len(self.all_ap) else 0.0
        # OLD VERSION:
    ...
    def map75(self):
        ...
        """
        # OWN MODIFICATION: changed to use weighted mean
        return self._mean(self.all_ap[:, 5], weighted=False) if len(self.all_ap) else 0.0
        # ONLY WEIGHTED VERSION:
        # return self._weighted_mean(self.all_ap[:, 5]) if len(self.all_ap) else 0.0
        # OLD VERSION:
    ...
    def map(self):
        ...
        """
        # OWN MODIFICATION: changed to use weighted mean
        return self._mean(self.all_ap.mean(1), weighted=False) if len(self.all_ap) else 0.0
        # ONLY WEIGHTED VERSION:
        # return self._weighted_mean(self.all_ap.mean(1)) if len(self.all_ap) else 0.0
        # OLD VERSION:
        # return self.all_ap.mean() if len(self.all_ap) else 0.0

    # def mean_results(self):
    #     """Return mean of results, mp, mr, map50, map, and F1.""" # OWN MODIFICATION: added F1
    #     # ONLY WEIGHTED VERSION:
    #     return [self.mp, self.mr, self._weighted_mean(self.f1), self.map50, self.map] # OWN MODIFICATION: added F1 to the return list
    #     return [self.mp, self.mr, self.map50, self.map]

    def mean_results(self, weighted=False):
        """
        Return mean evaluation metrics: precision, recall, F1, mAP@0.5, and mAP@0.5:0.95.
        """
        mean_fn = lambda x: self._mean(x, weighted=weighted)

        mp = mean_fn(self.p) if len(self.p) else 0.0
        mr = mean_fn(self.r) if len(self.r) else 0.0
        mf1 = mean_fn(self.f1) if len(self.f1) else 0.0
        map50 = mean_fn(self.all_ap[:, 0]) if len(self.all_ap) else 0.0
        map_5095 = mean_fn(self.all_ap.mean(1)) if len(self.all_ap) else 0.0

        return [mp, mr, mf1, map50, map_5095]

    def class_result(self, i):
        """Return class-aware result, p[i], r[i], f1[i], ap50[i], ap[i].""" # OWN MODIFICATION: added F1 to the return list
        return self.p[i], self.r[i], self.f1[i], self.ap50[i], self.ap[i] # OWN MODIFICATION: added F1 to the return list
    ...
    def fitness(self):
        ...
        # w = [0.0, 0.0, 0.0, 0.1, 0.9]   # weights for [P, R, F1, mAP@0.5, mAP@0.5:0.95] # OWN MODIFICATION: added F1 to the weights
        # w = [0.05, 0.05, 0.5, 0.3, 0.1]  
        w = [0.25, 0.25, 0.5, 0.0, 0.0]
        return (np.array(self.mean_results()) * w).sum()

    # def fitness(self):
    #     """Custom fitness function emphasizing class 1 performance."""
    #     if self.nc < 2:
    #         return 0.0  # Binary classification expected

    #     # Class 1 metrics
    #     p1, r1, f1_1, ap50_1, _ = self.class_result(1)

    #     # Fitness = 0.5 * F1 + 0.2 * Recall + 0.2 * Precision + 0.1 * mAP50 (all for class 1)
    #     return 0.5 * f1_1 + 0.2 * r1 + 0.2 * p1 + 0.1 * ap50_1


    def update(self, results, nt = None): # OWN MODIFICATION: added argument nt = None
        """
        ...
        self.nt = nt  # OWN MODIFICATION: Added nt to store nt for weighted mean
...
class DetMetrics(SimpleClass):
    ...
    def __init__(self, save_dir=Path("."), plot=False, names={}, class_counts=None) -> None:
        """
    ...
    def process(self, tp, conf, pred_cls, target_cls, on_plot=None):
        ...
        nt = np.array([np.sum(target_cls == c) for c in np.unique(target_cls)])  # OWN MODIFICATION: Added line to get per-class counts
        self.box.update(results, nt=nt) # OWN MODIFICATION: Added nt argument to update function

    @property
    def keys(self):
        """Return a list of keys for accessing specific metrics."""
        return ["metrics/precision(B)", "metrics/recall(B)", "metrics/F1(B)", "metrics/mAP50(B)", "metrics/mAP50-95(B)"] # OWN MODIFICATION: Added F1 to the keys

    # ONLY WEIGHTED VERSION:
    # def mean_results(self):
    #     """Calculate mean of detected objects & return precision, recall, mAP50, mAP50-95, and F1 score.""" 
    #     return self.box.mean_results()
    
    def mean_results(self, weighted=False):
        """Calculate mean of detected objects & return precision, recall, mAP50, mAP50-95, and F1 score.""" 
        return self.box.mean_results(weighted=weighted)
...
class SegmentMetrics(SimpleClass):
    ...
    def process(self, tp, tp_m, conf, pred_cls, target_cls, on_plot=None):
        ...
        self.seg.update(results_mask, nt=nt) # OWN MODIFICATION: Added nt argument to update function
        ...
        nt = np.array([np.sum(target_cls == c) for c in np.unique(target_cls)]) # OWN MODIFICATION: Added line to get per-class counts
        self.box.nc = len(self.names)
        self.box.update(results_box, nt=nt) # OWN MODIFICATION: Added nt argument to update function
    ...
    def keys(self):
        ...
        return [
            ...
            "metrics/mAP50(M)",
            "metrics/mAP50-95(M)",
        ] # OWN MODIFICATION: Added F1 to the keys
    ...
    def mean_results(self, weighted=False):
        """Return the mean metrics for bounding box and segmentation results."""
        return self.box.mean_results(weighted=weighted) + self.seg.mean_results(weighted=weighted)
...
class PoseMetrics(SegmentMetrics):
    ...
    def process(self, tp, tp_p, conf, pred_cls, target_cls, on_plot=None):
        ...
        self.pose.update(results_pose, nt=nt) # OWN MODIFICATION: Added nt argument to update function
        results_box = ap_per_class(
            tp,
            conf,
            pred_cls,
    ...
    def mean_results(self, weighted=False):
        """Return the mean results of box and pose."""
        return self.box.mean_results(weighted=weighted) + self.pose.mean_results(weighted=weighted)
